...
typedef struct {
    union {
        struct {
            uint32_t chroma_keyed : 1; /*1: The image contains transparent
                                          pixels with LV_COLOR_TRANSP color*/
    ...
    union {
        const uint8_t *pixel_map; /*For internal images (c arrays) pointer to
                                     the pixels array*/
        uint8_t first_pixel; /*For external images (binary) the first byte of
                                the pixels (just for convenient)*/
    };
} lv_img_t;
