...
class ADIEncoder : private ADIPort {
  public:
     ...
     * side UP \param port_bottom The "bottom" wire from the encoder sensor
     * \param reverse
     *        If "true", the sensor will count in the opposite direction
     */
    ADIEncoder(std::uint8_t port_top, std::uint8_t port_bottom,
               bool reversed = false);
...
class ADIUltrasonic : private ADIPort {
  public:
     ...
     * the next highest port following port_echo. \param port_echo The port
     * connected to the yellow INPUT cable. This should be in port 1, 3, 5, or 7
     * ('A', 'C', 'E', 'G').
     */
...
class ADIGyro : private ADIPort {
  public:
     ...
     * initialize() when the robot is stationary to ensure proper calibration.
     * If an ADIGyro object is declared at the global scope, a hardcoded 1300ms
